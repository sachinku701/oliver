...
class Pos_Bridge_User
{
     ...
     *
     */
    protected function set_customer_shipping_address( $param, $customer ) {
        $customer->set_shipping_first_name( isset( $param['first_name'] ) ? sanitize_text_field($param['first_name']) : '' );
        $customer->set_shipping_last_name( isset( $param['last_name'] ) ? sanitize_text_field($param['last_name']) : '' );
        $customer->set_shipping_address_1( isset( $param['address'] ) ? sanitize_text_field($param['address']) : '' );
        $customer->set_shipping_address_2( isset( $param['address2'] ) ? sanitize_text_field($param['address2']) : '' );
        $customer->set_shipping_city( isset( $param['city'] ) ? sanitize_text_field($param['city']) : '' );
        $customer->set_shipping_postcode( isset( $param['post_code'] ) ? sanitize_text_field($param['post_code']) : '' );
        $customer->set_shipping_country( isset( $param['country'] ) ? sanitize_text_field($param['country']) : '' );
        $customer->set_shipping_state( isset( $param['state'] ) ? sanitize_text_field($param['state']) : '' );
    }
